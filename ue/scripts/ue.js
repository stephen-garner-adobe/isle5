...
const MUTATING_BLOCK_SELECTOR = [
  ...
  'div.hero-cta',
  'div.newsletter',
  'div.promotional-hero',
  'div.top-banner',
].join(', ');

function hasInstrumentation(element) {
  return element?.nodeType === Node.ELEMENT_NODE
    && [...element.attributes].some(({ nodeName }) => (
      nodeName.startsWith('data-aue-') || nodeName.startsWith('data-richtext-')
    ));
}

function getInstrumentedElements(element) {
  if (!element?.querySelectorAll) return [];
  return [
    ...(hasInstrumentation(element) ? [element] : []),
    ...element.querySelectorAll('[data-aue-resource], [data-aue-prop], [data-richtext-prop]'),
  ];
}

function preserveReplacementInstrumentation(removedElements, addedElements) {
  if (!removedElements.length || !addedElements.length) return;

  if (removedElements.length === addedElements.length) {
    removedElements.forEach((removed, index) => {
      moveInstrumentation(removed, addedElements[index]);
    });
  }

  if (removedElements.length === 1 && addedElements.length === 1) {
    const removedInstrumented = getInstrumentedElements(removedElements[0]);
    const addedCandidates = [
      addedElements[0],
      ...addedElements[0].querySelectorAll('*'),
    ];

    removedInstrumented.forEach((instrumented, index) => {
      const target = addedCandidates[index];
      if (target) moveInstrumentation(instrumented, target);
    });
  }
}
...
const setupObservers = () => {
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      ...
      const mutatingBlock = target.closest?.(MUTATING_BLOCK_SELECTOR);
      if (mutation.type === 'childList' && target.tagName === 'DIV' && mutatingBlock) {
        const addedElements = [...mutation.addedNodes]
          .filter((node) => node.nodeType === Node.ELEMENT_NODE);
        const removedElements = [...mutation.removedNodes]
          .filter((node) => node.nodeType === Node.ELEMENT_NODE);

        preserveReplacementInstrumentation(removedElements, addedElements);
